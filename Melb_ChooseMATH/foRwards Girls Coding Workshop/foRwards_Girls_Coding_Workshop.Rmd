...
```{r echo=TRUE}
ggplot(diamonds_small) +
  geom_smooth(aes(x=carat, y=price, colour=cut), se=F)
```
...
To make it easier for others to explore a data set, now that you can an expert(!), we can put it into a web document, called a flexdashboard. You can learn more about them at the RStudio Rmarkdown flexdashboards [tutorial](http://rmarkdown.rstudio.com/flexdashboard/using.html). 

And you can see the example one tat we have built, by opening the file `pisa.Rmd` in your RStudio window. Click `Run document` and the app will run on your computer. Alternatively, you can view it by pointing your web browser to [https://visnut.shinyapps.io/pisa/](https://visnut.shinyapps.io/pisa/). Here's what you will see:
